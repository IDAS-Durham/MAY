...
class TestPersonInitialization:
    ...
    def test_init_basic(self, mock_geo_unit):
        ...
        assert person.activities == []
        assert person.properties == {}
        assert isinstance(person.activity_map, defaultdict)
    ...
    def test_init_with_activities(self, mock_geo_unit):
        ...
        assert person.activities == activities
...
class TestActivityManagement:
    ...
    def test_add_activity_duplicate_ignored(self, mock_geo_unit):
        ...
        assert person.activities.count('home') == 1
    ...
    def test_add_multiple_activities(self, mock_geo_unit):
        ...
        assert all(activity in person.activities for activity in activities_to_add)
...
class TestActivityMap:
    ...
    def test_activity_map_initialized(self, mock_geo_unit):
        """Test that activity_map is initialized as defaultdict."""
        person = Person(age=25, sex='male', geographical_unit=mock_geo_unit)

        assert isinstance(person.activity_map, defaultdict)

    def test_activity_map_default_behavior(self, mock_geo_unit):
        """Test that activity_map returns empty list for missing keys."""
        person = Person(age=25, sex='male', geographical_unit=mock_geo_unit)

        # Accessing non-existent key should return empty list
        assert person.activity_map['nonexistent'] == []

    def test_activity_map_can_store_values(self, mock_geo_unit):
        """Test that activity_map can store values."""
        person = Person(age=25, sex='male', geographical_unit=mock_geo_unit)
        ...
        person.activity_map['home'].append(mock_subset)

        assert len(person.activity_map['home']) == 1
        assert person.activity_map['home'][0] == mock_subset
...
class TestPersonStringRepresentation:
    ...
    def test_repr_different_ids(self, mock_geo_unit):
        """Test __repr__ shows different IDs for different people."""
        person1 = Person(age=25, sex='male', geographical_unit=mock_geo_unit)
        person2 = Person(age=30, sex='female', geographical_unit=mock_geo_unit)

        assert 'id=0' in repr(person1)
        assert 'id=1' in repr(person2)
...
class TestPersonIntegration:
    ...
    def test_multiple_people_independent(self, mock_geo_unit):
        ...
        # Verify independence
        assert person1.activities == ['home', 'work']
        assert person2.activities == ['work', 'home']
