...
def test_romantic_distributor_msoa_raking_shifts_lgb_share():
    """Two MSOAs with very different orientation marginals should produce
    very different LGB shares in the assigned population."""
    ...
    # Pick the MSOAs with the lowest and highest LGB share to get clear contrast.
    sorted_msoas = sorted(
        marginals.items(),
        key=lambda kv: kv[1]["homosexual"] + kv[1]["bisexual"],
    )
    low_code, low_marg = sorted_msoas[0]
    high_code, high_marg = sorted_msoas[-1]

    geo_low = MockGeoUnit(low_code)
    ...
    for _ in range(n_per_cell):
        for sex in ("male", "female"):
            for geo in (geo_low, geo_high):
                pid += 1
                people.append(MockPerson(pid, 35, sex, geographical_unit=geo))
    ...
    low_orients = Counter(
        p.properties["sexual_orientation"]
        for p in people
        if p.geographical_unit is geo_low
    )
    high_orients = Counter(
        p.properties["sexual_orientation"]
        for p in people
        if p.geographical_unit is geo_high
    )

    n_low = sum(low_orients.values())
    n_high = sum(high_orients.values())
    low_lgb = (low_orients["homosexual"] + low_orients["bisexual"]) / n_low
    high_lgb = (high_orients["homosexual"] + high_orients["bisexual"]) / n_high

    # The high-LGB MSOA must produce noticeably more LGB assignments than
    # the low-LGB MSOA — i.e. the geographic signal makes it through both
    # the IPF cell table and the vectorized sampling. We do NOT compare to
    # the raw MSOA marginal: this fixture spans only two cherry-picked
    # MSOAs whose aggregate LGB share is well above the national prior, so
    # IPF correctly dampens the absolute level toward the prior. What we
    # care about is preserved relative ordering.
    assert high_lgb > low_lgb + 0.02, (
        f"Expected high MSOA LGB > low MSOA LGB by ≥2pp; got {low_lgb:.3f} vs {high_lgb:.3f}"
    )
