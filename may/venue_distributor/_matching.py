...
class _MatchingMixin:
    def build_attribute_index(self, venues: List):
        ...
        for rule in active_rules:
            if rule["type"] == "categorical" and not rule["case_sensitive"]:
                rule["rules"] = {
                    ...
                    for k, vals in rule["rules"].items()
                }
                ...
                if rule["type"] == "numerical":
                    ...
                    if allowed:
                        for p_val in allowed:
                            index_key = (attr_name, self.owner._normalize_value(p_val))
        ...
        self.numerical_match_rules = [
            r for r in active_rules if r["type"] == "numerical"
        ]
        self.categorical_match_rules = [
            r
            for r in active_rules
            if r["type"] == "categorical" and not r.get("venue_column")
        ]
        ...
        if self.verbose:
            logger.info(
                f"Built attribute index for {len(venues)} venues with {len(attributes)} attributes"
            )
            if self.categorical_index:
                logger.info(
                    f"Built categorical index: {len(self.categorical_index)} unique value combinations"
                )

    def filter_venues_with_expansion(
        self,
        person,
        venues: List,
        initial_pool: List,
        ...
        person_attrs: Optional[Dict] = None,
    ) -> List:
        """
        Filter venues for a person.
        """
        # Tier 1: Try the initial pool (fast path)
        eligible = self.filter_venues_by_person(
            person, initial_pool, person_attrs=person_attrs
        )
        if eligible:
            return eligible
        ...
        if self.config.get("venue_selection", {}).get("consider_by") == "count":
            ...
            for search_count in search_limits:
                ...
                if self.verbose:
                    logger.debug(
                        f"Expanding search for person {person.id} to k={limit}"
                    )

                expanded_pool = self.owner._find_closest_venues(
                    location,
                ...
                eligible = self.filter_venues_by_person(
                    person, expanded_pool, person_attrs=person_attrs
                )
    ...
    def filter_venues_by_person(
        self, person, venues: List, person_attrs: Optional[Dict] = None
    ) -> List:
        """Filter venues based on person's attributes (age, gender, etc.)."""
        match_attrs = getattr(self.owner, "_pre_processed_match_attrs", [])

        # Step 1: Pre-filter using categorical index
        venues = self.prefilter_venues_by_categorical(
            person, venues, person_attrs=person_attrs
        )
        if not venues:
            return []
        ...
        eligible_venues = []
        for venue in venues:
            if self.venue_accepts_person(
                person, venue, match_attrs, person_attrs=person_attrs
            ):
                eligible_venues.append(venue)

        return eligible_venues

    def venue_accepts_person(
        self,
        person,
        venue,
    ...
    def prefilter_venues_by_categorical(
        self, person, venues: List, person_attrs: Optional[Dict] = None
    ) -> List:
        ...
        for rule in attributes:
            if rule.get("type") == "categorical" and rule.get("venue_column"):
                ...
                if val is not None:
                    ...
                    norm_val = self.owner._normalize_value(val)
    ...
    def select_venue(
        ...
        if strategy == "random":
            ...
            if len(valid_venues) < 50:
                return min(
                    ...
                    key=lambda v: self.owner._haversine_distance(
                        person_location, v.coordinates
                    ),
                )
            else:
                coords = np.array([v.coordinates for v in valid_venues])
                dists = self.owner._haversine_distance_vectorized(
                    person_location, coords
                )
            ...
            if len(valid) < 50:
                dists = [
                    self.owner._haversine_distance(person_location, v.coordinates)
                    for v in valid
                ]
            else:
            ...
            remaining_caps = np.array(
                [self.owner._get_remaining_capacity(v) for v in valid], dtype=np.float64
            )

            total_cap = max(self.owner._get_venue_capacity(valid[0]), 1)
            ...
            return max(venues, key=lambda v: self.owner._get_venue_capacity(v))
    ...
    def find_eligible_venues_for_location(
        ...
        if consider_by == "count":
            ...
            return self.owner._find_closest_venues(
                location,
                self.owner.venue_type,
                count,
                allowed_venue_ids=getattr(self.owner, "venue_ids", None),
            ...
            if valid:
                coords = np.array([v.coordinates for v in valid])
                dists = self.owner._haversine_distance_vectorized(location, coords)
    ...
    def _get_person_attr(
        self, person, attr_name: str, person_attrs: Optional[Dict]
    ) -> Any:
        """
    ...
    def _check_numerical_constraint(self, val, venue, rule: Dict) -> bool:
        ...
        min_v = (
            venue.properties.get(constraints.get("min_column"))
            if constraints.get("min_column")
            else None
        )
        max_v = (
            venue.properties.get(constraints.get("max_column"))
            if constraints.get("max_column")
            else None
        )
    ...
    def _check_categorical_constraint(self, val, venue, rule: Dict) -> bool:
        ...
        if not rule.get("case_sensitive", False):
            v_val = self.owner._normalize_value(v_val).lower()
            val = self.owner._normalize_value(val).lower()
            matching = {
                k.lower(): [self.owner._normalize_value(v).lower() for v in vals]
                for k, vals in matching.items()
            }
        else:
            v_val = self.owner._normalize_value(v_val)
            val = self.owner._normalize_value(val)
            matching = {
                k: [self.owner._normalize_value(v) for v in vals]
                for k, vals in matching.items()
            }
