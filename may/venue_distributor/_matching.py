...
class _MatchingMixin:
            ...
            if self.config.get('venue_selection', {}).get('consider_by') == 'count':
                ...
                for search_count in search_limits:
                    ...
                    expanded_pool = self.owner._find_closest_venues(
                        location, self.owner.venue_type, limit, 
        ...
        def select_venue(self, person, venues: List, person_location: Tuple[float, float]) -> Optional[Any]:
            ...
            if strategy == 'random':
                ...
                if len(valid_venues) < 50:
                    return min(valid_venues, key=lambda v: self.owner._haversine_distance(person_location, v.coordinates))
                else:
                    coords = np.array([v.coordinates for v in valid_venues])
                    dists = self.owner._haversine_distance_vectorized(person_location, coords)
                ...
                # Use scalar math for small sets, vectorized for large sets
                if len(valid) < 50:
                    dists = [self.owner._haversine_distance(person_location, v.coordinates) for v in valid]
                else:
                ...
                if len(valid) < 50:
                    dists = np.array([self.owner._haversine_distance(person_location, v.coordinates) for v in valid])
                else:
                    coords = np.array([v.coordinates for v in valid])
                    dists = self.owner._haversine_distance_vectorized(person_location, coords)
                ...
                remaining_caps = np.array([
                    self.owner._get_remaining_capacity(v) for v in valid
                ], dtype=np.float64)
        ...
        def find_eligible_venues_for_location(self, location: Tuple[float, float], venues: List) -> List:
            ...
            if consider_by == 'count':
                ...
                return self.owner._find_closest_venues(location, self.owner.venue_type, count, allowed_venue_ids=getattr(self.owner, 'venue_ids', None))
        ...
        def _check_categorical_constraint(self, val, venue, rule: Dict) -> bool:
            ...
            if not rule.get('case_sensitive', False):
                ...
                matching = {k.lower(): [self.owner._normalize_value(v).lower() for v in vals] for k, vals in matching.items()}
            else:
                v_val = self.owner._normalize_value(v_val)
                val = self.owner._normalize_value(val)
                matching = {k: [self.owner._normalize_value(v) for v in vals] for k, vals in matching.items()}
