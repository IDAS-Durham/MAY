...
class _AllocationMixin:
        def allocate_group(self, people: List, venues: List, allow_overflow: bool = False, group_search_limits=None) -> int:
            ...
            for person in people:
                geo = self.owner._get_geo_unit_at_level(person, self.owner.world, target_level=self.owner.batch_geo_level)
            ...
            for geo_unit, geo_people in people_by_geo.items():
                ...
                geo_nearby = self.owner._find_closest_venues((lat, lon), self.owner.venue_type, search_attempts[0], allowed_venue_ids=getattr(self.owner, 'venue_ids', None))
                
                if not allow_overflow and not self.owner._filter_venues_by_capacity(geo_nearby):
                ...
                for attr_vals, people_group in people_by_attrs.items():
                    ...
                    if eligible_venues:
                        for person in people_group:
                            venue = None
                            with_cap = self.owner._filter_venues_by_capacity(eligible_venues)
                            if with_cap:
                                # Selection strategy (e.g., closest) will pick from available venues
                                venue = self.select_venue(person, with_cap, (lat, lon))
                            elif allow_overflow:
                                venue = self.select_venue(person, eligible_venues, (lat, lon))
                            
                            if venue:
                                venue.add_to_subset(person, subset_key=self.owner.subset_key, 
                                                   activity_name=self.owner.activity_map_key, activity_type=self.owner.activity_type)
        ...
        def allocate_by_geo_unit(self, people: List, venues: List) -> List:
            ...
            for geo_unit, geo_people in people_by_geo.items():
                venue_search_unit = geo_unit if self.owner.batch_geo_level == self.owner.venue_geo_level else geo_unit.get_ancestor_by_level(self.owner.venue_geo_level)
                ...
                for attr_vals, group in attr_items:
                    ...
                    if cache_key not in pool_cache:
                        eligible_pool = venues_by_geo.get(venue_search_unit.name, []) if self.config.get('venue_selection', {}).get('consider_by') == 'geo_unit' else self.find_eligible_venues_for_location((lat, lon), venues)
                    ...
                    if respect_capacity:
                        available_venues = [v for v in p_venues if self.owner._get_remaining_capacity(v) > 0]
                    else:
                    ...
                    if strategy == 'random' and lat is not None:
                        ...
                        for person in group:
                            ...
                            while venue_ptr < len(available_venues):
                                ...
                                if not respect_capacity or self.owner._get_remaining_capacity(v) > 0:
                                    v.add_to_subset(person, subset_key=self.owner.subset_key, 
                                                  activity_name=self.owner.activity_map_key, activity_type=self.owner.activity_type)
                                    self.owner._increment_venue_count(v)
                                    allocated += 1
                                    assigned = True
                                    if respect_capacity and self.owner._get_remaining_capacity(v) <= 0:
                                        venue_ptr += 1
                                    break
                                else:
                                    venue_ptr += 1
                        ...
                        available_venues.sort(key=lambda v: self.owner._haversine_distance((lat, lon), self.owner._get_venue_location(v)))
                        venue_ptr = 0
                        ...
                        remaining = np.array(
                            [self.owner._get_remaining_capacity(v) for v in available_venues],
                            dtype=np.int64,
                        )
                        ...
                        venue_dists = np.array([
                            self.owner._haversine_distance((lat, lon), self.owner._get_venue_location(v))
                        ...
                        remaining_caps = np.array([
                            self.owner._get_remaining_capacity(v) if respect_capacity else 1
                            for v in available_venues
                        ], dtype=np.float64)
                        
                        total_cap = self.owner._get_venue_capacity(available_venues[0]) if available_venues else 1
                        ...
                        if self.verbose and geo_name not in self._cb_logged_geo:
                            ...
                            venue_counts = [self.owner.venue_capacity_tracker.get(id(v), 0) for v in available_venues]
                        ...
                        for person in group:
                            with_cap = [v for v in available_venues if not respect_capacity or self.owner._get_remaining_capacity(v) > 0]
                            if with_cap:
                                venue = self.select_venue(person, with_cap, (lat, lon) if lat is not None else None)
                                if venue:
                                    venue.add_to_subset(person, subset_key=self.owner.subset_key, 
                                                      activity_name=self.owner.activity_map_key, activity_type=self.owner.activity_type)
                                    self.owner._increment_venue_count(venue)
        ...
        def allocate_individual(self, people: List, venues: List) -> List:
            ...
            for i, person in enumerate(people, 1):
                ...
                p_venues = self.filter_venues_with_expansion(person, venues, pool, loc, search_limits)
