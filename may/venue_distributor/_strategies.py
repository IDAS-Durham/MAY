...
class _AllocationMixin:
    def allocate_group(
        ...
        search_limits = (
            ...
            else selection_config.get("search_limits", [target_count * 4])
        )
        if not search_limits:
            search_limits = [target_count * 4]
        ...
        for limit in search_limits:
            if limit is None:
                continue
            clean_limits.append(min(limit, 100))  # Absolute cap at 100 venues
        if not clean_limits:
            clean_limits = [20]
        ...
        for person in people:
            geo = self.owner._get_geo_unit_at_level(
                person, self.owner.world, target_level=self.owner.batch_geo_level
            )
            if geo:
                people_by_geo[geo].append(person)

        for geo_unit, geo_people in people_by_geo.items():
            if not (geo_unit.coordinates and len(geo_unit.coordinates) == 2):
                continue
            lat, lon = geo_unit.coordinates

            # Find nearby venues once per geo unit
            geo_nearby = self.owner._find_closest_venues(
                (lat, lon),
                self.owner.venue_type,
                search_attempts[0],
                allowed_venue_ids=getattr(self.owner, "venue_ids", None),
            )

            if not allow_overflow and not self.owner._filter_venues_by_capacity(
                geo_nearby
            ):
                people_processed += len(geo_people)
                self._log_progress(
                    people_processed, total_people, progress_interval, allocated_count
                )
                continue
            ...
            for attr_vals, people_group in people_by_attrs.items():
                ...
                eligible_venues = self.filter_venues_with_expansion(
                    person=people_group[0],
                    venues=venues,
                    initial_pool=geo_nearby,
                    location=(lat, lon),
                ...
                if eligible_venues:
                    for person in people_group:
                        venue = None
                        with_cap = self.owner._filter_venues_by_capacity(
                            eligible_venues
                        )
                        if with_cap:
                            # Selection strategy (e.g., closest) will pick from available venues
                            venue = self.select_venue(person, with_cap, (lat, lon))
                        elif allow_overflow:
                            venue = self.select_venue(
                                person, eligible_venues, (lat, lon)
                            )
    ...
    def allocate_by_geo_unit(self, people: List, venues: List) -> List:
        ...
        v_level = self.owner._require_venue_geo_level()
        for v in venues:
        ...
        respect_capacity = self.config.get("venue_selection", {}).get(
        ...
        for geo_unit, geo_people in people_by_geo.items():
            venue_search_unit = (
                geo_unit
                if self.owner.batch_geo_level == self.owner.venue_geo_level
                else geo_unit.get_ancestor_by_level(self.owner.venue_geo_level)
            )
            ...
            lat, lon = (
                geo_unit.coordinates
                if (geo_unit.coordinates and len(geo_unit.coordinates) == 2)
                else (None, None)
            )
            ...
            for attr_vals, group in attr_items:
                ...
                if cache_key not in pool_cache:
                    eligible_pool = (
                        venues_by_geo.get(venue_search_unit.name, [])
                        if self.config.get("venue_selection", {}).get("consider_by")
                        == "geo_unit"
                        else self.find_eligible_venues_for_location((lat, lon), venues)
                    )

                    if eligible_pool:
                        p_attrs = dict(zip(self.attribute_names, attr_vals))
                        pool_cache[cache_key] = self.filter_venues_by_person(
                ...
                if not p_venues:
                    ...
                    processed += len(group)
                    self._log_progress(
                        processed, total, interval, allocated, prefix="  "
                    )
                    continue
                ...
                if respect_capacity:
                    available_venues = [
                        v for v in p_venues if self.owner._get_remaining_capacity(v) > 0
                    ]
                else:
                ...
                if strategy == "random" and lat is not None:
                    ...
                    for person in group:
                        ...
                        while venue_ptr < len(available_venues):
                            ...
                            if (
                                ...
                                or self.owner._get_remaining_capacity(v) > 0
                            ):
                                v.add_to_subset(
                                    person,
                                    subset_key=self.owner.subset_key,
                                ...
                                self.owner._increment_venue_count(v)
                                allocated += 1
                                assigned = True
                                if (
                                    respect_capacity
                                    and self.owner._get_remaining_capacity(v) <= 0
                                ):
                                    venue_ptr += 1
                                break
                            else:
                    ...
                    available_venues.sort(
                        key=lambda v: self.owner._haversine_distance(
                            (lat, lon), self.owner._get_venue_location(v)
                        )
                    )
                    venue_ptr = 0
                    ...
                    remaining = np.array(
                        [
                            self.owner._get_remaining_capacity(v)
                    ...
                    venue_dists = np.array(
                        [
                            self.owner._haversine_distance(
                                (lat, lon), self.owner._get_venue_location(v)
                            )
                            for v in available_venues
                        ]
                    ...
                    geo_name = (
                        geo_unit.name if hasattr(geo_unit, "name") else str(geo_unit)
                    )
                    group_allocated = 0
                    ...
                    total_cap = (
                        self.owner._get_venue_capacity(available_venues[0])
                        if available_venues
                        else 1
                    )
                    ...
                    for person in group:
                        ...
                        chosen_idx = valid_indices[
                            np.random.choice(len(valid_indices), p=probs)
                        ]
                    ...
                    if self.verbose and geo_name not in self._cb_logged_geo:
                        ...
                        venue_counts = [
                            self.owner.venue_capacity_tracker.get(id(v), 0)
                            for v in available_venues
                        ]
                        used = sum(1 for c in venue_counts if c > 0)
                        logger.debug(
                            f"  [closest_balanced] {geo_name}: {len(group)} people -> "
                    ...
                    for person in group:
                        ...
                        if with_cap:
                            venue = self.select_venue(
                                person,
                                with_cap,
                                (lat, lon) if lat is not None else None,
                            )
                            if venue:
                                venue.add_to_subset(
                                    person,
        ...
        self.owner.allocated_this_run += allocated
        return unallocated
    ...
    def allocate_individual(self, people: List, venues: List) -> List:
        ...
        for i, person in enumerate(people, 1):
            loc = self.owner._get_person_location(person)
            if not loc:
                unallocated.append(person)
                continue

            pool = self.find_eligible_venues_for_location(loc, venues)
            # STRICT DEFAULT: Only use first limit or small pool
            search_limits = self.config.get("venue_selection", {}).get(
                "search_limits", [20]
            )
            p_venues = self.filter_venues_with_expansion(
                person, venues, pool, loc, search_limits
            )

            if p_venues:
                with_cap = self.owner._filter_venues_by_capacity(p_venues)
                if with_cap:
                    venue = self.select_venue(person, with_cap, loc)
                    if venue:
                        venue.add_to_subset(
                            person,
                            subset_key=self.owner.subset_key,
    ...
    def _log_progress(self, current, total, interval, count, prefix="    "):
        ...
        if curr_threshold > prev_threshold or current >= total:
            logger.info(
                f"{prefix}Progress: {current}/{total} people processed ({min(100, current/total*100):.1f}%) - {count} allocated"
            )
