...
class PropertyMatchingDistributor(BaseDistributor):
    ...
    def __init__(self, config_file: str = None, config_dict: Dict = None):
        ...
        if missing_keys:
            raise ValueError(
                f"PropertyMatchingDistributor missing required config keys: {missing_keys}"
            )
        ...
        logger.info(
            f"Initialized PropertyMatchingDistributor for types: {self.target_venue_types}"
        )
        logger.info(
            f"  Matching '{self.mapping_key}' (person) to '{self.venue_property}' (venue)"
        )
    ...
    def allocate(self, world):
        ...
        logger.info("=" * 60)
        logger.info(
            f"Starting PropertyMatchingDistributor: {', '.join(self.target_venue_types)}"
        )
        logger.info("=" * 60)
        ...
        logger.info(
            f"  Created lookup map for {len(venue_map)} venues using property '{self.venue_property}'"
        )
            ...
            if norm_val is not None:
                ...
                if venue:
                    ...
                    if subset_key:
                        ...
                        logger.warning(
                            f"Could not determine subset for person {person.id} in venue {venue.id}"
                        )
                        missed_count += 1
                else:
                    missed_count += 1
        ...
        if missed_count > 0:
            logger.warning(
                f"  Failed to find a venue for {missed_count:,} people with a mapping key"
            )
            sample_keys = list(missed_keys)[:10]
            logger.warning(
                f"  Sample of missing keys ({len(missed_keys)} unique): {sample_keys}"
            )
