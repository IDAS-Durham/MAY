...
class _SpecialCasesMixin:
    def handle_special_cases(
        self, people: List, venues: List, world
    ) -> Tuple[List, List]:
        """
        ...
        for venue in venues:
            if (
                hasattr(venue, "name")
                and hasattr(venue, "geographical_unit")
                and venue.geographical_unit
            ):
        ...
        if venue_index and self.verbose:
            logger.debug(
                f"Built special case venue index with {len(venue_index)} entries"
            )
    ...
    def matches_special_case(self, person, case: Dict) -> bool:
        ...
        if "person_residence_type" in condition:
            ...
            if (
                res_venue is None
                or not hasattr(res_venue, "type")
                or res_venue.type != required_type
            ):
                return False

        if "filters" in condition:
            filters = condition["filters"]
            if not self.person_matches_filters(person, filters):
                return False

        return True

    def allocate_special_case(
        self, person, case: Dict, venues: List, venue_index: Dict = None
    ) -> bool:
        ...
        if strategy:
            geo_unit = self.owner._get_geo_unit_at_level(person, self.owner.world)
                ...
                if strategy == "closest":
                    ...
                    for venue in venues:
                        if venue.coordinates and len(venue.coordinates) == 2:
                            dist = self.owner._haversine_distance(
                                loc, venue.coordinates
                            )
                            if dist < min_dist:
                                min_dist = dist
        ...
        if if_no_match == "error":
            raise ValueError(
                f"Special case allocation failed for person {person.id} with residence '{res_name}'"
            )
        elif if_no_match == "warn":
            logger.warning(
                f"Special case: No match found for person {person.id} with residence '{res_name}'"
            )
