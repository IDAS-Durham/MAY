...
class _FallbackMixin:
    def handle_fallbacks(self, unallocated_people: List, venues: List, world) -> List:
        ...
        logger.debug(
            f"Handling fallbacks for {len(unallocated_people)} people using strategy '{strategy}'"
        )
    ...
    def _relax_distance(self, people: List, venues: List, config: Dict) -> List:
        ...
        try:
            for i in range(max_iters):
                ...
                logger.info(
                    f"  Relaxation iteration {i+1}/{max_iters} (distance x{scale:.1f})..."
                )
    ...
    def _assign_closest(self, people: List, venues: List) -> List:
        ...
        remaining = []
        logger.debug(
            "  Assigning to closest venue regardless of eligibility or capacity..."
        )

        for person in people:
            location = self.owner._get_person_location(person)
            if location:
                closest = self.owner._find_closest_venues(
                    location, self.owner.venue_type, 1
                )
                if closest:
                    venue = closest[0]
                    venue.add_to_subset(
                    ...
                    logger.warning(
                        f"Could not find ANY venue for person {person.id} in assign_closest fallback"
                    )
        ...
        logger.info(
            f"  Fallback (assign_closest): {allocated_count}/{len(people)} placed"
        )
