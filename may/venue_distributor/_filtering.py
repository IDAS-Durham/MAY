...
class _FilteringMixin:
        def apply_global_filters(self, people: List) -> List:
            ...
            """
            # vectorized path
            if (hasattr(self.owner, 'population_arrays') and 
                self.owner.population_arrays and 
                len(people) > 1000 and 
                self.owner._can_vectorize_filters(self.owner._pre_processed_filters)):
                ...
                if len(indices) == len(people):
                    ...
                    survivors = self.owner.population_arrays['people'][filtered_indices].tolist()
