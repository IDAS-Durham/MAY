...
class GraphRelationshipBuilder:
    ...
    def build_all(self, store: bool = True) -> dict[int, list[int]]:
        ...
        # Set k (mean connections per person), clamped to max possible
        max_k = self.n_people - 1
        k = self.mean_connections_per_person

        # Ensure k doesn't exceed what's possible for the graph
        if k > max_k:
            logger.warning(f'Average connections {self.mean_connections_per_person} exceeds the max possible for the graph n_people-1={max_k}. Reducing to the max possible')
            k = max_k
