...
def load_world_from_hdf5(
    input_file, config_file="configs/2021/serialization_config.yaml", slim=False
):
    """
        ...
        if "geography" in f:
            ...
            try:
                geography = _load_geography(
                    f["geography"], config, geo_names, level_registry
                )
        ...
        if "population" in f:
            ...
            try:
                population = _load_population(
                    f["population"], geography, config, slim=slim
                )
            except Exception as e:
            ...
            logger.warning("World will be created without population")
        logger.info(
            f"Population created in {time.perf_counter() - laptime:.2f} seconds"
        )
        # Load Venues
        venue_manager = None
        if "venues" in f:
        ...
        if slim:
            ...
            if geography:
                ...
                try:
                    ...
                    logger.info(
                        f"Per-unit statistics computed for {len(unit_statistics)} units."
                    )
                except Exception as e:
        ...
        if slim:
            ...
            try:
                if population and venue_manager:
                    _load_relationships(
                        f[activity_group_name], population, venue_manager
                    )
                else:
                    logger.warning(
                        "Cannot load relationships: population or venues missing"
                    )
            except Exception as e:
...
def _compute_slim_statistics(f) -> dict:
    ...
    if activity_group_name in f and "activity_map" in f[activity_group_name]:
        ...
        activity_data = am["activity_data"][
            :
        ]  # (N_rows, 4): person_id, act_idx, venue_id, subset_idx
...
def _compute_unit_statistics(f, geography) -> dict:
    ...
    if "venues" in f:
        ...
        if (
            type_reg is not None
            and types_raw.dtype.kind in ("u", "i")
            and len(types_raw)
        ):
    ...
    def _add(dst: dict, src: dict) -> None:
        ...
        for label in AGE_LABELS:
            dst.setdefault("age_distribution", {})[label] = dst.get(
                "age_distribution", {}
            ).get(label, 0) + src.get("age_distribution", {}).get(label, 0)
...
def _load_population(pop_group, geography, config, slim=False):
            ...
            if prop_data.dtype.kind == "S" or prop_data.dtype.kind == "O":
                ...
                try:
                    ...
                    prop_data = [
                        ...
                        for val in prop_data.astype(str)
                    ]
                except Exception:
    ...
    for i, (person_id, age, sex, geo_unit_id) in enumerate(
        ...
        if (i + 1) % progress_interval == 0 or (i + 1) == num_people:
            ...
            logger.info(
                f"    Progress: {i:,}/{num_people:,} people loaded ({progress:.1f}%)"
            )
...
def _load_venues(
    ...
    for i, (venue_id, name, venue_type, geo_unit_id) in enumerate(
        ...
        if not slim and venue_type_str in properties_by_venue_type:
            for prop_name, prop_array in properties_by_venue_type[
                venue_type_str
            ].items():
    ...
    for venue_id, parent_id in zip(ids, parent_ids):
        if int(parent_id) != -1:
            venues_by_global_id[int(venue_id)].parent = venues_by_global_id[
                int(parent_id)
            ]
    ...
    if "subsets" in venues_group:
        subsets_by_venue_and_index = _load_subsets(
            venues_group["subsets"], venues_by_global_id, subset_names_arr, slim=slim
        )
...
def _load_subsets(
    subsets_group, venues_by_global_id, subset_names_arr=None, slim=False
):
    ...
    for i, (venue_id, subset_idx, subset_name) in enumerate(
        zip(venue_ids, subset_indices, subset_names)
    ):
        ...
        subset = Subset(
            venue=venue, subset_index=subset_idx_int, subset_name=subset_name_str
        )
...
def _load_relationships(rel_group, population, venue_manager):
    ...
    for person_idx in range(num_people):
        ...
        start_idx = int(activity_offsets[person_idx])
        end_idx = (
            int(activity_offsets[person_idx + 1])
            if person_idx + 1 < len(activity_offsets)
            else len(activity_data)
        )
        ...
        if (person_idx + 1) % progress_interval == 0 or person_idx + 1 == num_people:
            ...
            logger.info(
                f"    Progress: {person_idx + 1:,}/{num_people:,} people processed ({progress:.1f}%)"
            )
